...
class DeepLearningController:
    ...
    def pipeline(self, id_video):
        ...
        if video_exist:
            ...
            prediction=self.pipelineController.predecir_video(input_path_avi, output_path_avi)
            output_path_mp4=self.pass_from_avi_to_mp4(input_path_avi=output_path_avi,id_video=id_video)
            return {"success": True, "message": "El vídeo ha sido procesado con éxito",
                    "extras": {"local_path": output_path_mp4, "name_file": f"{id_video}_learned.mp4","has_violence":self.has_violence(prediction)}}
    ...
    def pass_from_avi_to_mp4(self, input_path_avi, id_video):
        print("buscando el archivo para el pase de avi a mp4")
        ouput_filename_mp4=f"{id_video}_learned.avi"
        output_filepath_mp4 = self.get_output_local_path_mp4(id_video=id_video)
        ffmpeg_path = "ffmpeg"
        if platform.system() == "Windows":
            ffmpeg_path = r"C:\ffmpeg\bin\ffmpeg.exe"
            # Comando de conversión con FFmpeg
        ffmpeg_command = [
            ffmpeg_path, "-y", "-i", input_path_avi, "-c:v", "libx264", "-crf", "23", "-preset", "medium", "-c:a", "aac",
            "-b:a", "192k", output_filepath_mp4
        ]
